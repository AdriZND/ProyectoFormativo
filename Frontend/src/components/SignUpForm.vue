...
const schema = yup.object().shape({
    ...
    passwordConfirmation: yup.string().label('').required()
...
<template>
    <form @submit="onSubmit">
        <div class="d-flex flex-column">
            ...
            <input v-model="name" v-bind="nameAttrs" id="name" type="text" class="form-control form-control-sm mb-2" />

            <label for="surnames" class="form-label h6">Surnames</label>
            <input v-model="surnames" v-bind="surnamesAttrs" id="surnames" class="form-control form-control-sm mb-2" />

            <label for="username" class="form-label h6">Username</label>
            <input v-model="username" v-bind="usernameAttrs" id="username" class="form-control form-control-sm mb-2" />

            <label for="email" class="form-label h6">Email</label>
            <input v-model="email" v-bind="emailAttrs" id="email" class="form-control form-control-sm mb-2" />

            <label for="repeatEmail" class="form-label h6">Confirm your email</label>
            <input v-model="repeatEmail" v-bind="repeatEmailAttrs" id="repeatEmail"
                class="form-control form-control-sm mb-2" />

            <label for="password" class="form-label h6">Password</label>
            <input v-model="password" v-bind="passwordAttrs" id="password" class="form-control form-control-sm mb-2"
                type="password" />

            <label for="passwordConfirmation" class="form-label h6">Repeat your password</label>
            <input v-model="passwordConfirmation" v-bind="passwordConfirmationAttrs" id="passwordConfirmation"
                class="form-control form-control-sm mb-2" type="password" />

            <label for="rol" class="for-label h6">Are you a teacher? Or a student.</label>
            <select class="form-select" id="rol" v-model="selectRole" v-bind="selectRoleAttrs" @change="setValues({token: ''})">
                <option value="Student">Student</option>
                <option value="Teacher">Teacher</option>
            </select>
            <div v-if="selectRole === 'Teacher'" class="d-flex flex-column mt-2">
                <label class="h6">Enter your access token</label>
                <input v-model="token" v-bind="tokenAttrs"class="form-control form-control-sm">
                <p>{{ errors.token }}</p>
            </div>

        </div>
        <div class="d-flex justify-content-center"><button type="submit" class="btn btn-secondary mt-3">Sign up</button>
        </div>
