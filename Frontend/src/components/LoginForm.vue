...
<template>
    <form @submit="onSubmit" class="form">
        <div class="mb-2 ">
            ...
            <input v-model="username" v-bind="usernameAttrs" class="form-control" id="username"
                placeholder="Enter your username" type="text" />


            <label for="password" class="form-label h5">Password</label>
            <input v-model="password" v-bind="passwordAttrs" type="password" id="password" class="form-control"
                placeholder="Enter your password" />


            <div class="d-flex justify-content-between">
                <button type="submit" class="btn btn-secondary mt-3">Login</button>
                <router-link to="/signup"><button class="btn btn-secondary mt-3">Sign up</button></router-link>
            </div>
